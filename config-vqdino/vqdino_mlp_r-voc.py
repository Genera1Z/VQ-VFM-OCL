...
model = dict(
    ...
    decode=dict(
        ...
        backbone=dict(
            type="MLP",
            in_dim=embed_dim,
            dims=[2048] * 3 + [embed_dim + vfm_dim + 1],  # 2048>1024, 256+c>4+c
            ln=None,
            dropout=0,
        ),
    ),
...
loss_fn = dict(
    recon0=dict(
        ...
        transform=dict(
            type="Compose",
            transforms=[
                dict(type="Slice1", keys=["input"], dim=1, start=0, end=embed_dim),
                dict(type="Detach", keys=["target"]),
            ],
        ),
        # TODO weight=0.5,
    ),
    recon=dict(
        ...
        transform=dict(
            ...
            transforms=[  # embed_dim:embed_dim+c
                dict(type="Slice1", keys=["input"], dim=1, start=embed_dim, end=None),
                dict(type="Detach", keys=["target"]),
            ],
        ),
    ),
